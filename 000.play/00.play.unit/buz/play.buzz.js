...
const openPlay = async (cpy, bal, ste) => {
  ...
  bit = await ste.bus( ActVsg.REMOVE_VISAGE, { idx: "vsg00" });
  bit = await ste.bus( ActVsg.MOUNT_VISAGE, { idx: "vsg00", src: "indexCanvas", dat: { height: 120 }});
  bit = await ste.bus( ActVsg.READ_VISAGE, { idx: "vsg00" });

  bit = await ste.bus( ActCan.WRITE_CONTAINER, { idx: "can00", src: 'vsg00' });
  bit = await ste.bus( ActCan.SURFACE_CONTAINER, { idx: 'fce-can-00', src: "vsg00" });

  if (bal.slv != null)
    return bal.slv({ plyBit: { idx: "open-play", dat } });
